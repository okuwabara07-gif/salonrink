...
export default async function DashboardPage() {
  ...
  const [{ data: reservations }, { data: customers }] = await Promise.all([
    supabase.from('reservations').select('id, customer_name, datetime, menu, status, price, completed_at').eq('salon_id', salon.id).order('datetime', { ascending: false }).limit(50),
