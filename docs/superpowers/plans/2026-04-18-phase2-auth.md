...
**依存バージョンポリシー:** `@supabase/supabase-js` は 2.x 系最新、`@supabase/ssr` は最新安定版（0.10.x 以降、getAll/setAll cookie API 対応が条件）。具体的なバージョン番号は明記しない。
